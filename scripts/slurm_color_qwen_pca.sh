...
python scripts/generate_llm_dataset.py --domain colors --config configs/llm/color_sweep.yml

echo "[$(date '+%Y-%m-%d %H:%M:%S')] Phase 2: Running orchestrator active learning sweeps (Qwen PCA)..."
python scripts/run_experiment_orchestration.py --config configs/llm/color_sweep.yml
