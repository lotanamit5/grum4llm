...
def test_asymptotic_runner_is_reproducible_for_seed() -> None:
    ...
    a = run_asymptotic_social_choice([5, 10], dataset_name="dataset2", repeats=2, seed=9, mcem_config=cfg)
    b = run_asymptotic_social_choice([5, 10], dataset_name="dataset2", repeats=2, seed=9, mcem_config=cfg)

    assert [(p.n_agents, p.social_tau) for p in a] == [(p.n_agents, p.social_tau) for p in b]
...
def test_criteria_comparison_returns_expected_keys_and_range() -> None:
    ...
    score_dict = compare_criteria_social_choice(dataset_name="dataset2", criterion_name="social", n_rounds=5, repeats=2, seed=3, mcem_config=cfg)

    assert isinstance(score_dict, dict)
    score = score_dict["social"]
...
def test_non_random_criterion_can_outperform_random_on_dataset2() -> None:
    ...
    score_random = compare_criteria_social_choice(dataset_name="dataset2", criterion_name="random", n_rounds=8, repeats=1, seed=5, mcem_config=cfg)["social"]
    score_social = compare_criteria_social_choice(dataset_name="dataset2", criterion_name="social", n_rounds=8, repeats=1, seed=5, mcem_config=cfg)["social"]
    score_opt = compare_criteria_social_choice(dataset_name="dataset2", criterion_name="d_opt", n_rounds=8, repeats=1, seed=5, mcem_config=cfg)["social"]
...
def test_dataset_selector_changes_output_distribution() -> None:
    ...
    a = run_asymptotic_social_choice([5, 10], dataset_name="dataset1", repeats=1, seed=5, mcem_config=cfg)
    b = run_asymptotic_social_choice([5, 10], dataset_name="dataset2", repeats=1, seed=5, mcem_config=cfg)
...
def test_invalid_dataset_raises_value_error() -> None:
    cfg = MCEMConfig(n_iterations=0, n_gibbs_samples=5, n_gibbs_burnin=2, random_seed=2)
    try:
        _ = run_asymptotic_social_choice([5], dataset_name="nope", repeats=1, seed=5, mcem_config=cfg)
...
def test_asymptotic_parallel_matches_serial_for_fixed_seed() -> None:
    ...
    assert [(p.n_agents, p.social_tau) for p in serial] == [(p.n_agents, p.social_tau) for p in parallel]
...
def test_social_choice_elicitation_curve_has_one_checkpoint_per_observation_count() -> None:
    ...
    assert -1.0 <= curve[-1].social_tau <= 1.0
