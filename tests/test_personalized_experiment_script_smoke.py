...
def test_run_personalized_choice_experiment_script_smoke(tmp_path: Path) -> None:
    root = Path(__file__).resolve().parents[1]
    script = root / "scripts" / "run_personalized_choice_experiment.py"
    out = tmp_path / "personalized_choice_results.json"
...
def test_run_with_config_file(tmp_path: Path) -> None:
    root = Path(__file__).resolve().parents[1]
    script = root / "scripts" / "run_personalized_choice_experiment.py"
    out = tmp_path / "personalized_choice_results_cfg.json"
